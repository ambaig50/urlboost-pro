...
async function fetchPageSpeed(url: string, strategy: 'mobile'|'desktop') {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      if (attempt > 0) await new Promise(r => setTimeout(r, 3000)); // 3s retry delay
      const r = await fetch(endpoint, { cache:'no-store', signal: AbortSignal.timeout(25000) });
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const uniqueInternal = [...new Set(
    allLinks
      .filter(l => l.includes(hn) || /href=["']\/(?!\/)/.test(l))
      .map(l => l.replace(/href=["']/,'').replace(/["']/,'').trim())
      .filter(l =>
        l &&
        !l.startsWith('#') &&
        !l.startsWith('mailto:') &&
        ...
        !l.includes('/_next/') &&
        !l.includes('/static/') &&
        !l.match(/\.(css|js|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|map|json)(\?|$)/i)
      )
      .map(l => {
        // Normalise relative links to full URL
        if (l.startsWith('/')) return `${parsedUrl.origin}${l}`;
        return l;
      })
  )].slice(0, 15);
  ...
  // Check canonical points to the same domain being analysed
  const canonicalHost = canonical ? (() => { try { return new URL(canonical, parsedUrl.origin).hostname; } catch { return ''; } })() : '';
  const canonicalOk = !!canonical && canonicalHost === hn;
  chk(canonicalOk, 'Canonical tag present', canonical||'Set.',
    canonical ? 'Canonical points to wrong domain' : 'Missing canonical tag',
    canonical ? `Canonical URL points to ${canonicalHost} instead of ${hn}.` : 'No canonical URL tag.',
    canonical ? `Update canonical tag to use https://${hn}/ instead of the current domain.` : 'Add <link rel="canonical">.',
    'warnings');
  ...
  // Check sitemap points to the same domain being analysed
  const sitemapHost = sitemapUrl ? (() => { try { return new URL(sitemapUrl).hostname; } catch { return ''; } })() : '';
  const sitemapOk = hasSitemap && sitemapHost === hn;
  chk(sitemapOk, 'XML sitemap found', sitemapUrl||'Found.',
    hasSitemap ? 'Sitemap points to wrong domain' : 'No XML sitemap',
    hasSitemap ? `Sitemap URL points to ${sitemapHost} instead of ${hn}.` : 'No sitemap.xml detected.',
    hasSitemap ? `Update sitemap references to use https://${hn}/ instead of the current domain.` : 'Create and submit sitemap.xml.',
    'warnings');

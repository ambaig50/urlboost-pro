...
export default function URLBoostPro() {
  ...
  const downloadReport = () => {
    ...
    // ═══════════════════════════════════════════════════
    // PAGE 2 — Client Explanation (2 columns for GOOD items, full width for WARN/CRIT)
    // ═══════════════════════════════════════════════════
    doc.addPage(); darkBg();
    ...
    // Score summary — single compact line
    let p2y = 38;
    ...
    doc.setDrawColor(40,55,75); doc.line(14,p2y+3,196,p2y+3); p2y+=7;
    ...
    if (goodIssues.length > 0) {
      ...
      goodIssues.forEach(issue => {
        ...
        const noteLines = wrap(exp?.goodNote||issue.message, 80, 6.5);
        const cardH = 8 + noteLines.length*4;
        ...
        doc.setFontSize(6.5); doc.setTextColor(52,211,153);
        doc.text(decode(issue.message).slice(0,38), cx+4, cy+5);
        doc.setTextColor(160,185,165); doc.setFontSize(6);
        noteLines.forEach((line,i)=>doc.text(line, cx+4, cy+10+i*4));
    ...
    // WARNING/CRITICAL items — full width with all 3 sections
    if (actionIssues.length > 0) {
      ...
      actionIssues.forEach(issue => {
        ...
        if (p2y+blockH > 278) { doc.addPage(); darkBg(); p2y=10; }
    ...
    if (p2y+stepsH>278){doc.addPage();darkBg();p2y=10;}
    ...
    // Disclaimer
    if (p2y+13>278){doc.addPage();darkBg();p2y=10;}
    doc.setFillColor(18,26,40); doc.roundedRect(14,p2y,182,13,2,2,'F');
    doc.setFontSize(6); doc.setTextColor(80,100,130);
    const disc='Disclaimer: Based on static HTML analysis at time of scanning. JavaScript-rendered content may not be captured. SEO results depend on competition, content quality, and backlinks. This is a starting-point guide, not a substitute for a full professional SEO audit.';

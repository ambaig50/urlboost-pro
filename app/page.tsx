...
const DEFAULT_SITES: SubmissionSiteExt[] = [
  ...
  { id: 'archive',     name: 'Wayback Machine',         url: 'https://web.archive.org/save/',              desc: 'Archives your page instantly — crawlable by all engines.',    category: 'Archive',          appendUrl: true,  requiresAccount: false, fields: ['url'],                                    instructions: 'Page opens with your URL pre-filled. Click "Save Page" button.' },
  { id: 'google-ping', name: 'Google Sitemap Ping',     url: 'https://www.google.com/ping?sitemap=',       desc: 'Pings Google to trigger a re-crawl of your sitemap.',         category: 'Search Engine',    appendUrl: true,  requiresAccount: false, fields: ['sitemap_url'],                            instructions: 'Browser loads the ping URL directly — Google confirms with a 200 OK response.' },
  { id: 'bing-ping',   name: 'Bing Sitemap Ping',       url: 'https://www.bing.com/ping?sitemap=',         desc: 'Pings Bing & Yahoo to re-index your sitemap.',               category: 'Search Engine',    appendUrl: true,  requiresAccount: false, fields: ['sitemap_url'],                            instructions: 'Browser loads the ping URL directly — Bing confirms with a 200 OK response.' },

  // ── Free Directories (no account needed) ──
  { id: 'exactseek',   name: 'ExactSeek',               url: 'https://www.exactseek.com/add.html',         desc: 'Free submission form — no account needed.',                   category: 'Directory',        appendUrl: false, requiresAccount: false, fields: ['url','title','description'],              instructions: 'Paste your URL, title, and description into the form. Click Submit.' },
  ...
  { id: 'sonicrun',    name: 'SonicRun',                url: 'https://www.sonicrun.com/freelisting.html',  desc: 'Free web directory listing — no account needed.',            category: 'Directory',        appendUrl: false, requiresAccount: false, fields: ['url','title','description'],              instructions: 'Fill in URL, title, and a short description. Click Add URL.' },
  { id: 'viesearch',   name: 'Viesearch',               url: 'https://viesearch.com/submit',               desc: 'Human-reviewed directory — no login required.',              category: 'Directory',        appendUrl: false, requiresAccount: false, fields: ['url','title','description'],              instructions: 'Enter URL, title, and description. Select a category. Click Submit.' },
  { id: 'prolinkdir',  name: 'ProLinkDirectory',        url: 'https://www.prolinkdirectory.com/submit-url.php', desc: 'Free directory — no signup needed.',                   category: 'Directory',        appendUrl: false, requiresAccount: false, fields: ['url','title','description'],              instructions: 'Fill in URL, title, description, and category. Click Submit URL.' },
  { id: 'somuch',      name: 'SoMuch.com',              url: 'https://www.somuch.com/submit-links/',       desc: 'High DA general directory — free, no account.',              category: 'Directory',        appendUrl: false, requiresAccount: false, fields: ['url','title','description'],              instructions: 'Enter your link details and click Submit.' },
  { id: 'jayde',       name: 'Jayde.com',               url: 'https://www.jayde.com/addurl.html',          desc: 'Business web directory — free, no registration.',            category: 'Directory',        appendUrl: false, requiresAccount: false, fields: ['url','title','description'],              instructions: 'Fill in URL, title, and description. Click Add URL.' },
  ...
  { id: 'yelp',        name: 'Yelp for Business',       url: 'https://biz.yelp.com/claiming/search',       desc: 'Claim or create your free Yelp business page.',              category: 'Business Listing', appendUrl: false, requiresAccount: true,  fields: ['business_name','address','phone','website','category'], instructions: '1. Search for your business name\n2. If found, click "Claim this business"\n3. If not found, click "Add your business"\n4. Create a free Yelp account\n5. Verify ownership by phone' },
  { id: 'foursquare',  name: 'Foursquare for Business', url: 'https://business.foursquare.com/',           desc: 'Foursquare & Swarm listing — location data used by many apps.', category: 'Business Listing', appendUrl: false, requiresAccount: true,  fields: ['business_name','address','phone','website'],            instructions: '1. Click "Claim your venue"\n2. Search for your business\n3. Create a free account\n4. Add or confirm business details\n5. Verify via phone or email' },
];
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  {(guideSite.fields.length > 1 || guideSite.requiresAccount) && (
                    <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-4 mb-4">
                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-3 uppercase tracking-wide">📋 Copy these details into the form</p>
                      <div className="space-y-2">
                        {guideSite.fields.includes('url') && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Website URL</p>
                            <p className="text-sm font-medium text-blue-600 dark:text-blue-400 break-all select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{seoData.url}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('title') && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Title / Business Name</p>
                            <p className="text-sm font-medium select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{bizInfo.businessName || seoData.title || new URL(seoData.url).hostname}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('description') && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Description</p>
                            <p className="text-sm select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">{bizInfo.description || `Visit ${new URL(seoData.url).hostname} — ${seoData.title}`}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('business_name') && bizInfo.businessName && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Business Name</p>
                            <p className="text-sm font-medium select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{bizInfo.businessName}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('address') && bizInfo.address && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Address</p>
                            <p className="text-sm select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{bizInfo.address}{bizInfo.city ? `, ${bizInfo.city}` : ''}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('phone') && bizInfo.phone && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Phone</p>
                            <p className="text-sm select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{bizInfo.phone}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('email') && bizInfo.email && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Email</p>
                            <p className="text-sm select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{bizInfo.email}</p>
                          </div>
                        )}
                        {guideSite.fields.includes('category') && bizInfo.category && (
                          <div>
                            <p className="text-xs text-slate-400 mb-0.5">Category</p>
                            <p className="text-sm select-all bg-white dark:bg-slate-900 rounded px-2 py-1 border border-slate-200 dark:border-slate-700">{bizInfo.category}</p>
                          </div>
                        )}
                        {(!bizInfo.businessName && guideSite.requiresAccount) && (
                          <p className="text-xs text-amber-500">💡 Fill in your business info above to see pre-filled details here.</p>
                        )}
                      </div>
                    </div>
                  )}

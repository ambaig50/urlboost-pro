...
export default function URLBoostPro() {
  ...
  const downloadFullAudit = () => {
    ...
    // Priority fixes
    if (a.executive.priorityFixes.length > 0) {
      doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Priority Fixes', 14, y); y+=6;
      a.executive.priorityFixes.forEach((fix: string, i: number) => {
        doc.setFillColor(22,33,52); doc.roundedRect(14,y-3,182,9,1,1,'F');
        doc.setFillColor(96,165,250); doc.circle(19,y+1.5,2.5,'F');
        doc.setTextColor(15,23,42); doc.setFontSize(6.5); doc.text(String(i+1),19,y+2.5,{align:'center'});
        doc.setTextColor(180,195,215); doc.setFontSize(7.5); doc.text(decode(fix).slice(0,90), 24, y+3); y+=11;
      });
    }
    ...
    const techRows: [string, string, boolean][] = [
      ...
      ['Canonical URL',     a.technical.hasCanonical ? decode(a.technical.canonicalUrl).slice(0,65) : 'Not set — duplicate content risk', a.technical.hasCanonical],
      ['XML Sitemap',       a.technical.hasSitemap ? decode(a.technical.sitemapUrl).slice(0,60) : 'Not found — submit to Search Console', a.technical.hasSitemap],
    ...
    const socialRows: [string, string, boolean][] = [
      ['Open Graph', a.onPage.hasOpenGraph ? (a.onPage.ogTitle?decode(a.onPage.ogTitle).slice(0,50):'Present') : 'Not found', a.onPage.hasOpenGraph],
    ...
    if(a.issues.passed.length>0){
      if(y>260){newPage();y=20;}
      doc.setFontSize(7.5); doc.setTextColor(52,211,153); doc.text(`PASSED (${a.issues.passed.length})`, 14, y); y+=5;
      // Full-width passed items
      a.issues.passed.forEach((iss: {item:string;detail:string}) => {
        // Use full width for passed items so text doesn't truncate
        if(y>272){newPage();y=20;}
        const detL = wrap(decode(iss.detail).slice(0,90), 148, 6.5);
        const ph = 8 + detL.length*4;
        ...
        detL.forEach((l:string,i:number)=>doc.text(l,19,y+11+i*4));
        y+=ph+3;
      });
      y+=4;
    }
    ...
    actionSections.forEach(([title,col,items])=>{
      if(y+items.length*8+12>278){newPage();y=20;}
      doc.setFillColor(22,33,52); doc.roundedRect(14,y,182,items.length*8+10,2,2,'F');
      doc.setFillColor(...col); doc.rect(14,y,3,items.length*8+10,'F');
      doc.setFontSize(8); doc.setTextColor(255,255,255); doc.text(title,20,y+7);
      doc.setFontSize(7); doc.setTextColor(180,195,215);
      items.forEach((item,i)=>doc.text(item.slice(0,90),20,y+14+i*8));
      y+=items.length*8+16;
    });

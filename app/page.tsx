...
export default function URLBoostPro() {
  ...
  const downloadFullAudit = () => {
    ...
    if (!a.performance.available) {
      doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,20,2,2,'F');
      doc.setFontSize(9); doc.setTextColor(148,163,184);
      doc.text('PageSpeed data unavailable — Google API rate limited or unreachable.', 18, y+8);
      doc.text('Run again in a few minutes, or check manually at pagespeed.web.dev', 18, y+16);
      y+=28;
    } else {
      // Performance score
    ...
    if(a.issues.critical.length>0){
      ...
      a.issues.critical.forEach((iss: {item:string;detail:string;fix:string}) => {
        ...
        const detailLines = wrap(decode(iss.detail), 150, 7);
        const fixLines    = wrap('Fix: '+decode(iss.fix), 150, 7);
        const bh = 10 + detailLines.length*5 + fixLines.length*5 + 14;
        ...
        let by = y+15;
        doc.setFontSize(6); doc.setTextColor(248,113,113); doc.text('WHAT IT MEANS', 20, by); by+=4;
        doc.setTextColor(210,180,180); doc.setFontSize(7);
        detailLines.forEach((l:string)=>{doc.text(l,20,by);by+=5;});
        by+=2;
    ...
    // Warnings — with full detail + fix explanation
    if(a.issues.warnings.length>0){
      ...
      a.issues.warnings.forEach((iss: {item:string;detail:string;fix:string}) => {
        ...
        let by = y+15;
        // What it means
        doc.setFontSize(6); doc.setTextColor(251,191,36); doc.text('WHAT IT MEANS', 20, by); by+=4;
    ...
    if(a.issues.passed.length>0){
      ...
      const colW=89; let ci=0; let c0y=y; let c1y=y;
      a.issues.passed.forEach((iss: {item:string;detail:string}) => {
        const cx = ci===0?14:14+colW+4; const cy = ci===0?c0y:c1y;
        if(cy>272){newPage(); c0y=20; c1y=20; ci=0;}
        doc.setFillColor(12,40,28); doc.roundedRect(cx,cy,colW,13,1,1,'F');
        doc.setFillColor(52,211,153); doc.rect(cx,cy,2,13,'F');
        doc.setTextColor(52,211,153); doc.setFontSize(6.5); doc.text(decode(iss.item).slice(0,30),cx+5,cy+6);
        doc.setTextColor(148,163,184); doc.setFontSize(6); doc.text(decode(iss.detail).slice(0,35),cx+5,cy+11);
        if(ci===0){c0y+=16;ci=1;}else{c1y+=16;ci=0;}
      });
      y=Math.max(c0y,c1y)+4;
    }

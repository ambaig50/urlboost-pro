...
export default function URLBoostPro() {
  ...
  const downloadReport = () => {
    ...
    doc.setFontSize(7); doc.setTextColor(100,116,139); doc.text('PAGE TITLE', 14, y1+16);
    doc.setFontSize(9.5); doc.setTextColor(220,230,245);
    doc.text(decode(seoData.title||'No title found').slice(0,85), 14, y1+23);

    doc.setDrawColor(40,55,75); doc.setLineWidth(0.4); doc.line(14,y1+30,196,y1+30);
    ...
    // ═══════════════════════════════════════════════════
    // PAGE 2 — Detailed Explanation (fits in ONE page)
    // ═══════════════════════════════════════════════════
    doc.addPage(); darkBg();

    // Header
    doc.setFillColor(30,41,59); doc.rect(0,0,W,34,'F');
    doc.setFillColor(...scoreRgb); doc.rect(0,0,4,34,'F');
    doc.setTextColor(255,255,255); doc.setFontSize(13);
    doc.text('Detailed SEO Explanation & Recommendations', 14, 12);
    doc.setFontSize(7.5); doc.setTextColor(148,163,184);
    doc.text(`Prepared for: ${hostname}  |  ${dateStr}`, 14, 20);
    doc.text('Plain-English explanation of each finding with actionable next steps for your team or developer.', 14, 27);

    // Score summary — compact single-line style
    let p2y = 41;
    ...
    const interpBody = sc>=70
      ? `A score of ${sc}/100 means the core SEO fundamentals are in good shape. Search engines can read and index your page effectively. Focus on the warnings below and build quality backlinks from reputable websites in your industry.`
      : sc>=40
      ? `A score of ${sc}/100 means the basics are partially covered but gaps exist that are reducing your search visibility. The flagged issues below are straightforward to fix and could meaningfully improve how often your page appears in search results.`
      : `A score of ${sc}/100 means search engines are struggling to index your page properly. This directly impacts whether your site appears in search results at all. The critical issues below should be treated as urgent and fixed within the next few days.`;

    const interpLines = wrap(interpBody, 165, 7.5);
    const interpH = 12 + interpLines.length*4.8;
    ...
    doc.text(interpTitle, 20, p2y+7);
    doc.setFontSize(7.5); doc.setTextColor(180,195,215);
    interpLines.forEach((line,i) => doc.text(line, 20, p2y+13+i*4.8));
    p2y += interpH+7;
    ...
    const explanations: Record<string,{why:string;how:string;impact:string;goodNote:string}> = {
      'Title tag':{
        goodNote:'Your title tag is well-optimised and the right length. It will display correctly in Google search results without being cut off.',
        why:'The title tag is the blue clickable headline in Google results and the most important on-page SEO element.',
        how:'Write a unique title per page, 50-60 characters, with your primary keyword near the front.',
        impact:'A poor title can cut click-through rates by 30-50% and weakens rankings for target keywords.',
      },
      'Meta description':{
        goodNote:'Your meta description is present and a good length. Google will show users a relevant preview of your page before they click.',
        why:'The meta description appears below your title in search results and directly influences whether users click.',
        how:'Write 150-160 characters with your main keyword and a clear reason to click — treat it as a short ad.',
        impact:'A compelling description can increase click-through rates by 5-15% with no extra ad spend.',
      },
      'H1 tag':{
        goodNote:'Your page has exactly one H1 tag. Search engines can clearly identify your page\'s main topic.',
        why:'The H1 is the main headline. Each page should have exactly one so search engines understand the topic.',
        how:'Add one H1 with your primary keyword. Use H2 and H3 for sub-sections.',
        impact:'Missing or multiple H1 tags confuse search engines and reduce keyword rankings.',
      },
      'Images':{
        goodNote:'All images have alt text or there are no images on the page. No SEO issues here.',
        why:'Image alt text is read by search engines since they cannot see images. It also supports screen reader users.',
        how:'Add short descriptive alt attributes to every image including a keyword where natural.',
        impact:'Missing alt text means search engines skip your images, losing Google Image Search traffic.',
      },
      'Viewport':{
        goodNote:'Your viewport tag is correctly set, signalling mobile-friendliness to Google — a positive ranking factor.',
        why:'The viewport tag tells mobile browsers how to scale your page. Without it, your site may appear broken on phones.',
        how:'Add: meta name="viewport" content="width=device-width, initial-scale=1" inside your head tag.',
        impact:'Google ranks the mobile version of your site first. A missing viewport tag directly hurts rankings.',
      },
      'Structured data':{
        goodNote:'Schema.org structured data is detected. Your page is eligible for rich results in Google such as star ratings and FAQs.',
        why:'Schema.org data tells search engines exactly what your content is — product, business, article etc — enabling rich results.',
        how:'Add a JSON-LD script block in your head describing your content type. Use Google\'s Structured Data Markup Helper.',
        impact:'Rich results can double your listing size in search results and significantly increase clicks.',
      },
      'Canonical':{
        goodNote:'Your canonical tag is set correctly. Search engines know which URL to rank and your page authority is not being diluted.',
        why:'A canonical tag tells search engines which URL is the definitive version, preventing duplicate content issues.',
        how:'Add link rel="canonical" in your page head pointing to the preferred URL with correct protocol and trailing slash.',
        impact:'Without it, search engines may split your authority across URL variants, reducing all their rankings.',
      },
      'Open Graph':{
        goodNote:'Open Graph tags are present. Your page will show an attractive preview image and description when shared on social media.',
        ...
        impact:'Without these tags, social shares show a broken preview, reducing trust and clicks from social media.',
      },
    };

    // Render issue cards — GOOD items compact, WARNING/CRITICAL full
    seoData.issues.forEach(issue => {
      ...
      const col:[number,number,number] = issue.type==='good'?[52,211,153]:issue.type==='warning'?[251,191,36]:[248,113,113];
      const label = issue.type==='good'?'GOOD':issue.type==='warning'?'WARN':'CRIT';
      const isGood = issue.type==='good';

      // Calculate height
      const noteLines = wrap(isGood ? exp.goodNote : exp.why, 155, 7);
      const howLines  = isGood ? [] : wrap(exp.how, 155, 7);
      const impLines  = isGood ? [] : wrap(exp.impact, 155, 7);
      const allLines  = noteLines.length + howLines.length + impLines.length;
      const sections  = isGood ? 1 : 3;
      const blockH    = 10 + allLines*4.5 + sections*6 + (isGood?0:4);

      if (p2y+blockH > 278) {
        // Should not happen with tight layout but safety net
        doc.addPage(); darkBg(); p2y=10;
      }

      doc.setFillColor(20,30,46); doc.roundedRect(14,p2y,182,blockH,2,2,'F');
      doc.setFillColor(...col); doc.rect(14,p2y,3,blockH,'F');

      // Title + badge
      doc.setFontSize(8); doc.setTextColor(255,255,255);
      doc.text(decode(issue.message).slice(0,82), 20, p2y+7);
      doc.setFillColor(...col); doc.roundedRect(168,p2y+1.5,22,8,2,2,'F');
      doc.setTextColor(15,23,42); doc.setFontSize(6);
      doc.text(label, 179, p2y+7, {align:'center'});

      let by = p2y+13;

      if (isGood) {
        doc.setFontSize(6); doc.setTextColor(...col); doc.text('WHAT THIS MEANS', 20, by); by+=4.5;
        doc.setTextColor(180,195,215); doc.setFontSize(7);
        noteLines.forEach(line=>{doc.text(line,20,by);by+=4.5;});
      } else {
        doc.setFontSize(6); doc.setTextColor(...col); doc.text('WHY IT MATTERS', 20, by); by+=4.5;
        doc.setTextColor(180,195,215); doc.setFontSize(7);
        noteLines.forEach(line=>{doc.text(line,20,by);by+=4.5;}); by+=3;

        doc.setFontSize(6); doc.setTextColor(...col); doc.text('HOW TO FIX IT', 20, by); by+=4.5;
        doc.setTextColor(180,195,215); doc.setFontSize(7);
        howLines.forEach(line=>{doc.text(line,20,by);by+=4.5;}); by+=3;

        doc.setFontSize(6); doc.setTextColor(...col); doc.text('BUSINESS IMPACT', 20, by); by+=4.5;
        doc.setTextColor(180,195,215); doc.setFontSize(7);
        impLines.forEach(line=>{doc.text(line,20,by);by+=4.5;});
      }

      p2y += blockH+3;
    });

    // Next steps — compact inline numbered list
    p2y+=3;
    doc.setDrawColor(40,55,75); doc.line(14,p2y,196,p2y); p2y+=6;
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Recommended Next Steps', 14, p2y); p2y+=6;

    const steps = [
      '1. Fix CRITICAL issues within 7 days — they have the biggest impact on search visibility.',
      '2. Address WARNING items next — most take under an hour with your developer or CMS.',
    ...
    doc.setFillColor(20,30,46); doc.roundedRect(14,p2y-3,182,steps.length*7+4,2,2,'F');
    doc.setFontSize(7.5); doc.setTextColor(180,195,215);
    steps.forEach((step,i) => { doc.text(step, 18, p2y+i*7); });
    p2y += steps.length*7+8;

    // Disclaimer
    doc.setFillColor(18,26,40); doc.roundedRect(14,p2y,182,14,2,2,'F');
    doc.setFontSize(6); doc.setTextColor(80,100,130);
    const disc = 'Disclaimer: This report is based on static HTML analysis at the time of scanning. JavaScript-rendered content may not be fully captured. SEO results depend on competition, content quality, and backlinks. This is a starting-point guide, not a substitute for a full professional SEO audit.';
    wrap(disc,172,6).forEach((line,i) => doc.text(line,18,p2y+5+i*4));

...
export default function URLBoostPro() {
  ...
  const downloadFullAudit = () => {
    ...
    // Title
    doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,24,2,2,'F');
    doc.setFillColor(...titleCol); doc.rect(14,y,3,24,'F');
    doc.setFontSize(7); doc.setTextColor(148,163,184); doc.text('TITLE TAG', 20, y+7);
    doc.setFontSize(8); doc.setTextColor(220,230,245);
    wrap(decode(a.onPage.title)||'Not found', 158, 8).slice(0,2).forEach((l: string,i: number)=>doc.text(l,20,y+14+i*6));
    doc.setTextColor(...titleCol); doc.setFontSize(7);
    doc.text(`${a.onPage.titleLength} chars — ${a.onPage.titleScore==='good'?'Optimal (50-60)':a.onPage.titleLength>60?'Too long':'Too short'}`, 20, y+21);
    y+=28;

    // Meta desc
    doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,24,2,2,'F');
    doc.setFillColor(...descCol); doc.rect(14,y,3,24,'F');
    doc.setFontSize(7); doc.setTextColor(148,163,184); doc.text('META DESCRIPTION', 20, y+7);
    doc.setFontSize(8); doc.setTextColor(220,230,245);
    wrap(decode(a.onPage.metaDesc)||'Not found', 158, 8).slice(0,2).forEach((l: string,i: number)=>doc.text(l,20,y+14+i*6));
    doc.setTextColor(...descCol); doc.setFontSize(7);
    doc.text(`${a.onPage.metaDescLength} chars — ${a.onPage.metaDescScore==='good'?'Optimal (150-160)':a.onPage.metaDescLength>160?'Too long':'Too short or missing'}`, 20, y+21);
    y+=28;
    ...
    if(a.issues.critical.length>0){
      ...
      a.issues.critical.forEach((iss: {item:string;detail:string;fix:string}) => {
        ...
        const lines = wrap(decode(iss.fix), 145, 6.5);
        const bh = 10+lines.length*4;
        doc.setFillColor(40,12,12); doc.roundedRect(14,y,182,bh,1,1,'F');
        doc.setFillColor(248,113,113); doc.rect(14,y,2,bh,'F');
        doc.setTextColor(220,230,245); doc.setFontSize(7.5); doc.text(decode(iss.item).slice(0,70),19,y+6);
        doc.setTextColor(148,163,184); doc.setFontSize(6.5);
        lines.forEach((l:string,i:number)=>doc.text(l,19,y+11+i*4));
        y+=bh+3;
      });
      y+=3;
    }
    ...
    if(a.issues.warnings.length>0){
      ...
      a.issues.warnings.forEach((iss: {item:string;detail:string;fix:string}) => {
        ...
        const lines = wrap(decode(iss.fix), 145, 6.5);
        const bh = 10+lines.length*4;
        doc.setFillColor(40,30,10); doc.roundedRect(14,y,182,bh,1,1,'F');
        doc.setFillColor(251,191,36); doc.rect(14,y,2,bh,'F');
        doc.setTextColor(220,230,245); doc.setFontSize(7.5); doc.text(decode(iss.item).slice(0,70),19,y+6);
        doc.setTextColor(148,163,184); doc.setFontSize(6.5);
        lines.forEach((l:string,i:number)=>doc.text(l,19,y+11+i*4));
        y+=bh+3;

...
type AuditStep = 'idle' | 'loading' | 'done';
...
export default function URLBoostPro() {
  ...
  // Load logo as base64 for PDF headers
  useEffect(() => {
    fetch('/logo.jpg')
      .then(r => r.blob())
      .then(blob => {
  ...
  useEffect(() => {
    try {
      ...
      setDark(isDark);
      // Clear any submissions stored before v2 (guide-confirm flow) was introduced
      const version = localStorage.getItem('urlboost_version');
      if (version !== '2') {
        localStorage.removeItem(STORAGE_KEY);
        localStorage.setItem('urlboost_version', '2');
      }
  ...
  const clearHistory = () => {
    if (!seoData) return;
    const remaining = submissions.filter(r => r.submittedUrl !== seoData.url);
  ...
  const downloadSubmissionReport = () => {
    ...
    // Header
    doc.setFillColor(30,41,59); doc.rect(0,0,W,46,'F');
    doc.setFillColor(96,165,250); doc.rect(0,0,4,46,'F');
    if (logoBase64) { try { doc.addImage(logoBase64,'JPEG',14,5,34,34,undefined,'FAST'); } catch{} }
    const sTxtX = logoBase64 ? 53 : 14;
    doc.setTextColor(255,255,255); doc.setFontSize(17);
    doc.text('URLBoost Pro', sTxtX, 14);
    doc.setFontSize(7); doc.setTextColor(212,175,55);
    doc.text('by Ambaig Designs', sTxtX, 21);
    doc.setFontSize(8.5); doc.setTextColor(148,163,184);
    doc.text('Directory Submission Report', sTxtX, 29);
    doc.text(`Website: ${hostname}`, sTxtX, 36);
    doc.text(`Generated: ${dateStr}`, sTxtX, 43);

    // Summary box
    doc.setFillColor(15,23,42); doc.roundedRect(148,5,52,32,5,5,'F');
    doc.setFontSize(22); doc.setTextColor(96,165,250);
    doc.text(`${subs.length}`, 174, 26, {align:'center'});
    doc.setFontSize(7.5); doc.setTextColor(148,163,184);
    doc.text('Sites Submitted', 149, 33);

    // URL
    let y = 52;
    doc.setFontSize(7); doc.setTextColor(100,116,139); doc.text('SUBMITTED URL', 14, y);
    doc.setFontSize(10); doc.setTextColor(96,165,250);
    doc.text(seoData.url.slice(0,90), 14, y+7); y+=18;

    // Business info if available
    if (bizInfo.businessName) {
      doc.setFontSize(7); doc.setTextColor(100,116,139); doc.text('BUSINESS', 14, y);
      doc.setFontSize(9); doc.setTextColor(220,230,245);
      doc.text(bizInfo.businessName + (bizInfo.category ? ` — ${bizInfo.category}` : ''), 14, y+7); y+=16;
    }

    doc.setDrawColor(40,55,75); doc.setLineWidth(0.4); doc.line(14,y,196,y); y+=8;

    if (subs.length === 0) {
      doc.setFontSize(12); doc.setTextColor(148,163,184);
      doc.text('No submissions recorded yet for this URL.', 14, y+20);
    } else {
      // Table header
      doc.setFontSize(10); doc.setTextColor(255,255,255);
      doc.text(`Submission Log — ${subs.length} ${subs.length === 1 ? 'site' : 'sites'} submitted`, 14, y); y+=8;

      doc.setFillColor(22,33,52); doc.rect(14,y-4,182,8,'F');
      doc.setFontSize(7.5); doc.setTextColor(148,163,184);
      doc.text('No.', 16, y+1);
      doc.text('Site Name', 26, y+1);
      doc.text('Category', 100, y+1);
      doc.text('Submitted To', 130, y+1);
      doc.text('Date', 163, y+1);
      ...
      subs.forEach((sub, i) => {
        ...
        const siteDef = [...DEFAULT_SITES].find(s => s.id === sub.siteId);
        const cat = siteDef?.category || 'Directory';
        if (i % 2 === 0) { doc.setFillColor(20,30,46); doc.rect(14,y-4,182,8,'F'); }
        doc.setTextColor(148,163,184); doc.setFontSize(7);
        doc.text(String(i+1), 18, y, {align:'center'});
        doc.setTextColor(220,230,245); doc.setFontSize(7.5);
        doc.text(sub.siteName.slice(0,32), 26, y);
        doc.setTextColor(100,116,139); doc.setFontSize(7);
        doc.text(cat.slice(0,18), 100, y);
        doc.setTextColor(96,165,250); doc.setFontSize(7);
        doc.text(sub.submittedTo.replace('https://','').replace('http://','').slice(0,24), 130, y);
        doc.setTextColor(148,163,184);
        doc.text(new Date(sub.date).toLocaleDateString(), 163, y);
        doc.setTextColor(52,211,153);
        doc.text('Posted', 185, y);
        y += 8;
      });
      ...
      doc.setFontSize(10); doc.setTextColor(255,255,255); doc.text('Summary', 14, y); y+=8;

      const cats = ['Search Engine','Archive','Directory','Business Listing'];
      const catCounts = cats.map(c => ({
        cat: c,
        count: subs.filter(s => {
          const def = DEFAULT_SITES.find(d => d.id === s.siteId);
      ...
      catCounts.forEach((c, i) => {
        const x = 14 + i * (boxW + 4);
        doc.setFillColor(22,33,52); doc.roundedRect(x, y, boxW, 20, 3, 3, 'F');
        doc.setFontSize(14); doc.setTextColor(96,165,250);
        doc.text(String(c.count), x + boxW/2, y+12, {align:'center'});
        doc.setFontSize(6); doc.setTextColor(100,116,139);
        doc.text(c.cat, x + boxW/2, y+18, {align:'center'});
      });
      y += 30;

      // Next submission tips
      if (y < 240) {
        doc.setDrawColor(40,55,75); doc.line(14,y,196,y); y+=8;
        doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Next Steps', 14, y); y+=7;
        const tips = [
          '• Allow 2-8 weeks for directories to review and list your submission.',
          '• Log in to Google Search Console and Bing Webmaster Tools to verify indexing.',
          '• Run a new SEO analysis in 4 weeks to track improvement in your score.',
          '• Consider re-submitting to business listings quarterly to keep info current.',
        ];
        doc.setFontSize(7.5); doc.setTextColor(180,195,215);
        tips.forEach(tip => { doc.text(tip, 14, y); y += 6; });
      }
    }

    // Footer
    doc.setFontSize(6.5); doc.setTextColor(40,55,75);
    doc.text(`Submission Report  |  ${hostname}  |  URLBoost Pro  |  urlboost-pro.vercel.app`, W/2, 291, {align:'center'});

    doc.save(`URLBoost_Submissions_${hostname}_${new Date().toISOString().slice(0,10)}.pdf`);
  };
  ...
  const runFullAudit = async () => {
    const trimmed = url.trim();
    if (!trimmed.startsWith('http://') && !trimmed.startsWith('https://')) {
      setAuditError('Please enter a valid URL starting with http:// or https://'); return;
    }
    setAuditStep('loading'); setAuditError(''); setAuditProgress(0);
    const iv = setInterval(() => setAuditProgress(p => Math.min(85, p + 2)), 400);
    try {
      const res = await fetch('/api/fullaudit', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ url: trimmed }) });
      const data = await res.json();
      if (!res.ok || data.error) { setAuditError(data.error ?? 'Audit failed.'); setAuditStep('idle'); return; }
      setAuditData(data); setAuditProgress(100);
      setTimeout(() => setAuditStep('done'), 400);
    } catch { setAuditError('Network error. Please try again.'); setAuditStep('idle'); }
  ...
  const downloadFullAudit = () => {
    ...
    const dateStr = new Date(a.timestamp).toLocaleDateString('en-GB',{day:'2-digit',month:'long',year:'numeric'});
    const decode = (s: string) => (s||'').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&#39;/g,"'").replace(/&quot;/g,'"');
    const darkBg = () => { doc.setFillColor(15,23,42); doc.rect(0,0,W,297,'F'); };
    const wrap = (t: string, mw: number, fs: number) => { doc.setFontSize(fs); return doc.splitTextToSize(t,mw) as string[]; };
    const scoreRgb = (s: number): [number,number,number] => s>=70?[52,211,153]:s>=40?[251,191,36]:[248,113,113];
    const sRgb = scoreRgb(a.executive.score);

    let pageNum = 1;
    const newPage = () => {
      doc.setFontSize(6.5); doc.setTextColor(40,55,75);
      doc.text(`Page ${pageNum}  |  Full SEO Audit  |  ${hostname}  |  urlboost-pro.vercel.app`, W/2, 291, {align:'center'});
      doc.addPage(); darkBg(); pageNum++;
    };

    // ── PAGE 1: Executive Summary ──────────────────────────────────────────
    darkBg();
    doc.setFillColor(30,41,59); doc.rect(0,0,W,50,'F');
    doc.setFillColor(...sRgb); doc.rect(0,0,5,50,'F');
    if (logoBase64) { try { doc.addImage(logoBase64,'JPEG',14,6,36,36,undefined,'FAST'); } catch{} }
    const aTxtX = logoBase64 ? 56 : 14;
    doc.setTextColor(255,255,255); doc.setFontSize(16);
    doc.text('URLBoost Pro — Full SEO Audit', aTxtX, 14);
    doc.setFontSize(7); doc.setTextColor(212,175,55);
    doc.text('by Ambaig Designs', aTxtX, 21);
    doc.setFontSize(8); doc.setTextColor(148,163,184);
    doc.text(`Website: ${hostname}`, aTxtX, 28);
    doc.text(`Analyzed: ${a.url}`, aTxtX, 34);
    doc.text(`Date: ${dateStr}`, aTxtX, 40);
    doc.text(`Powered by PageSpeed Insights + HTML Analysis`, aTxtX, 46);

    // Grade circle
    doc.setFillColor(22,33,52); doc.circle(185, 25, 16, 'F');
    doc.setFontSize(22); doc.setTextColor(...sRgb);
    doc.text(a.executive.grade, 185, 29, {align:'center'});
    doc.setFontSize(7); doc.setTextColor(148,163,184);
    doc.text('GRADE', 185, 37, {align:'center'});

    let y = 55;
    // Score bar
    doc.setFontSize(10); doc.setTextColor(255,255,255); doc.text('Executive Summary', 14, y); y+=7;
    doc.setFillColor(22,33,52); doc.roundedRect(14,y,182,28,3,3,'F');
    doc.setFillColor(...sRgb); doc.rect(14,y,3,28,'F');

    doc.setFontSize(26); doc.setTextColor(...sRgb); doc.text(`${a.executive.score}`, 32, y+18);
    doc.setFontSize(8); doc.setTextColor(148,163,184); doc.text('/100 Overall SEO Score', 52, y+18);

    const critLen = a.issues.critical.length, warnLen = a.issues.warnings.length, passLen = a.issues.passed.length;
    doc.setFontSize(8); doc.setTextColor(248,113,113); doc.text(`${critLen} Critical`, 110, y+10);
    doc.setTextColor(251,191,36); doc.text(`${warnLen} Warnings`, 110, y+18);
    doc.setTextColor(52,211,153); doc.text(`${passLen} Passed`, 110, y+26);
    y += 35;

    // Major issues
    if (a.executive.majorIssues.length > 0) {
      doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Major Issues Found', 14, y); y+=6;
      a.executive.majorIssues.forEach((issue: string) => {
        doc.setFillColor(40,12,12); doc.roundedRect(14,y-3,182,9,1,1,'F');
        doc.setFillColor(248,113,113); doc.rect(14,y-3,2,9,'F');
        doc.setTextColor(220,230,245); doc.setFontSize(8); doc.text(decode(issue), 19, y+3); y+=11;
      });
      y+=3;
    }

    // Priority fixes — show issue name + fix action
    const allActionIssues = [...a.issues.critical, ...a.issues.warnings];
    if (allActionIssues.length > 0) {
      doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Priority Fixes', 14, y); y+=6;
      allActionIssues.slice(0,6).forEach((iss: {item:string;fix:string}, i: number) => {
        const isCrit = i < a.issues.critical.length;
        const col: [number,number,number] = isCrit ? [248,113,113] : [251,191,36];
        doc.setFillColor(isCrit?40:35, isCrit?12:28, 10); doc.roundedRect(14,y-3,182,13,1,1,'F');
        doc.setFillColor(...col); doc.circle(20,y+3,3,'F');
        doc.setTextColor(15,23,42); doc.setFontSize(6.5); doc.text(String(i+1),20,y+4.5,{align:'center'});
        doc.setTextColor(255,255,255); doc.setFontSize(7.5); doc.text(decode(iss.item).slice(0,75), 26, y+3);
        doc.setTextColor(148,163,184); doc.setFontSize(6.5); doc.text(decode(iss.fix).slice(0,85), 26, y+9);
        y+=15;
      });
    }
    ...
    doc.setFillColor(96,165,250); doc.rect(0,0,5,22,'F');
    doc.setTextColor(255,255,255); doc.setFontSize(13);
    doc.text('Technical SEO Audit', 14, 14);
    y = 30;

    // Check if canonical/sitemap point to wrong domain (e.g. Vercel preview URL)
    const analyzedHost = new URL(a.url).hostname;
    const canonicalHost = a.technical.canonicalUrl ? (() => { try { return new URL(a.technical.canonicalUrl).hostname; } catch { return ''; } })() : '';
    const sitemapHost   = a.technical.sitemapUrl   ? (() => { try { return new URL(a.technical.sitemapUrl).hostname;   } catch { return ''; } })() : '';
    const canonicalMismatch = canonicalHost && canonicalHost !== analyzedHost;
    const sitemapMismatch   = sitemapHost   && sitemapHost   !== analyzedHost;

    const techRows: [string, string, boolean][] = [
      ['HTTPS / SSL',   a.technical.isHttps ? 'Enabled — secure connection' : 'NOT enabled — insecure HTTP', a.technical.isHttps],
      ['HTTP Status',   `${a.technical.statusCode} — ${a.technical.statusCode===200?'OK':a.technical.statusCode===301?'Redirect':a.technical.statusCode===404?'Not Found':'See code'}`, a.technical.statusCode===200||a.technical.statusCode===301],
      ['Canonical URL', a.technical.hasCanonical
        ? (canonicalMismatch
            ? `Points to WRONG domain (${canonicalHost})`
            : decode(a.technical.canonicalUrl).slice(0,65))
        : 'Not set — duplicate content risk',
        a.technical.hasCanonical && !canonicalMismatch],
      ['XML Sitemap',   a.technical.hasSitemap
        ? (sitemapMismatch
            ? `Points to WRONG domain (${sitemapHost})`
            : decode(a.technical.sitemapUrl).slice(0,60))
        : 'Not found — submit to Search Console',
        a.technical.hasSitemap && !sitemapMismatch],
      ['Viewport Tag',      a.technical.hasViewport ? 'Present — mobile-friendly' : 'Missing — mobile ranking impact', a.technical.hasViewport],
      ['Language Attr',     a.technical.hasLangAttr ? `lang="${a.technical.langAttr}"` : 'Not set on <html> tag', a.technical.hasLangAttr],
      ['Charset',           a.technical.hasCharset ? 'Declared' : 'Not declared', a.technical.hasCharset],
      ['Hreflang',          a.technical.hasHreflang ? 'Tags present' : 'Not set (OK for single language)', true],
      ['Robots Meta',       a.technical.hasRobotsMeta ? a.technical.robotsContent : 'Not set (defaults to index, follow)', true],
    ];
    techRows.forEach(([label, value, good], i) => {
      const col: [number,number,number] = good ? [52,211,153] : [248,113,113];
      if(i%2===0){doc.setFillColor(20,30,46);}else{doc.setFillColor(18,27,42);}
      doc.roundedRect(14,y-4,182,10,1,1,'F');
      doc.setFillColor(...col); doc.rect(14,y-4,2,10,'F');
      doc.setTextColor(148,163,184); doc.setFontSize(7); doc.text(label, 19, y+2);
      if(good){doc.setTextColor(52,211,153);}else{doc.setTextColor(248,113,113);} doc.setFontSize(7.5);
      doc.text(value.slice(0,80), 65, y+2);
      y+=12;
    });
    ...
    newPage();
    doc.setFillColor(30,41,59); doc.rect(0,0,W,22,'F');
    doc.setFillColor(52,211,153); doc.rect(0,0,5,22,'F');
    doc.setTextColor(255,255,255); doc.setFontSize(13);
    doc.text('On-Page SEO Analysis', 14, 14);
    y = 30;

    const titleCol: [number,number,number] = a.onPage.titleScore==='good'?[52,211,153]:a.onPage.titleScore==='warning'?[251,191,36]:[248,113,113];
    const descCol:  [number,number,number] = a.onPage.metaDescScore==='good'?[52,211,153]:a.onPage.metaDescScore==='warning'?[251,191,36]:[248,113,113];

    // Title — dynamic height
    const titleLines = wrap(decode(a.onPage.title)||'Not found', 155, 8);
    const titleBoxH = Math.max(24, 14 + titleLines.length * 6 + 6);
    doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,titleBoxH,2,2,'F');
    doc.setFillColor(...titleCol); doc.rect(14,y,3,titleBoxH,'F');
    doc.setFontSize(7); doc.setTextColor(148,163,184); doc.text('TITLE TAG', 20, y+7);
    doc.setFontSize(8); doc.setTextColor(220,230,245);
    titleLines.forEach((l: string, i: number) => doc.text(l, 20, y+14+i*6));
    doc.setTextColor(...titleCol); doc.setFontSize(7);
    doc.text(`${a.onPage.titleLength} chars — ${a.onPage.titleScore==='good'?'Optimal (50-60)':a.onPage.titleLength>60?'Too long':'Too short'}`, 20, y+titleBoxH-4);
    y += titleBoxH + 4;

    // Meta desc — dynamic height
    const descText = decode(a.onPage.metaDesc)||'Not found';
    const descLines = wrap(descText, 155, 8);
    const descBoxH = Math.max(24, 14 + descLines.length * 6 + 6);
    doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,descBoxH,2,2,'F');
    doc.setFillColor(...descCol); doc.rect(14,y,3,descBoxH,'F');
    doc.setFontSize(7); doc.setTextColor(148,163,184); doc.text('META DESCRIPTION', 20, y+7);
    doc.setFontSize(8); doc.setTextColor(220,230,245);
    descLines.forEach((l: string, i: number) => doc.text(l, 20, y+14+i*6));
    doc.setTextColor(...descCol); doc.setFontSize(7);
    doc.text(`${a.onPage.metaDescLength} chars — ${a.onPage.metaDescScore==='good'?'Optimal (150-160)':a.onPage.metaDescLength>160?'Too long':'Too short or missing'}`, 20, y+descBoxH-4);
    y += descBoxH + 4;

    // Headings
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Heading Structure', 14, y); y+=6;
    const hRows: [string, number, string][] = [['H1',a.onPage.h1Count,a.onPage.h1Text[0]??''],['H2',a.onPage.h2Count,a.onPage.h2Text[0]??''],['H3',a.onPage.h3Count,'']];
    hRows.forEach(([tag, count, sample]) => {
      const good = tag==='H1' ? count===1 : count>0;
      doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,10,1,1,'F');
      doc.setFillColor(good?52:248, good?211:113, good?153:113); doc.rect(14,y-3,2,10,'F');
      doc.setTextColor(148,163,184); doc.setFontSize(7.5); doc.text(`${tag} Tags:`, 19, y+3);
      doc.setTextColor(220,230,245); doc.text(`${count} found${sample?' — '+decode(sample).slice(0,60):''}`, 45, y+3);
      y+=12;
    });
    y+=4;

    // Images
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Images', 14, y); y+=6;
    const imgRows: [string, string, boolean][] = [
      ['Total Images', String(a.onPage.totalImages), true],
      ['With Alt Text', String(a.onPage.imagesWithAlt), a.onPage.imagesMissingAlt===0],
      ['Missing Alt Text', String(a.onPage.imagesMissingAlt), a.onPage.imagesMissingAlt===0],
    ];
    imgRows.forEach(([label, value, good]) => {
      doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,9,1,1,'F');
      const c: [number,number,number] = good?[52,211,153]:[248,113,113];
      doc.setFillColor(...c); doc.rect(14,y-3,2,9,'F');
      doc.setTextColor(148,163,184); doc.setFontSize(7); doc.text(label,19,y+2);
      doc.setTextColor(...c); doc.text(value,80,y+2);
      y+=11;
    });
    y+=4;

    // Social / Schema
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Social & Structured Data', 14, y); y+=6;
    const socialRows: [string, string, boolean][] = [
      ['Open Graph', a.onPage.hasOpenGraph ? (a.onPage.ogTitle?decode(a.onPage.ogTitle).slice(0,70):'Present') : 'Not found', a.onPage.hasOpenGraph],
      ['Twitter Card', a.onPage.hasTwitterCard ? 'Present' : 'Not found', a.onPage.hasTwitterCard],
      ['Schema.org', a.onPage.hasSchema ? (a.onPage.schemaTypes.join(', ')||'Detected') : 'Not found', a.onPage.hasSchema],
    ];
    socialRows.forEach(([label, value, good]) => {
      doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,9,1,1,'F');
      const c: [number,number,number] = good?[52,211,153]:[248,113,113];
      doc.setFillColor(...c); doc.rect(14,y-3,2,9,'F');
      doc.setTextColor(148,163,184); doc.setFontSize(7); doc.text(label,19,y+2);
      doc.setTextColor(220,230,245); doc.text(value.slice(0,65),60,y+2);
      y+=11;
    });
    y+=4;
    doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,18,2,2,'F');
    doc.setFontSize(7); doc.setTextColor(148,163,184);
    doc.text('Content',19,y+7); doc.text('Internal Links',65,y+7); doc.text('External Links',110,y+7); doc.text('Word Count',155,y+7);
    const wc: [number,number,number] = a.onPage.wordCount>=300?[52,211,153]:[248,113,113];
    doc.setTextColor(220,230,245); doc.text(String(a.internalLinking.totalInternalLinks),65,y+14);
    doc.text(String(a.onPage.externalLinks),110,y+14);
    doc.setTextColor(...wc); doc.text(`${a.onPage.wordCount} words`,155,y+14);

    // ── PAGE 4: Performance ────────────────────────────────────────────────
    newPage();
    doc.setFillColor(30,41,59); doc.rect(0,0,W,22,'F');
    doc.setFillColor(251,191,36); doc.rect(0,0,5,22,'F');
    doc.setTextColor(255,255,255); doc.setFontSize(13);
    doc.text('Site Performance & Core Web Vitals', 14, 14);
    y = 30;

    if (!a.performance.available) {
      doc.setFillColor(20,30,46); doc.roundedRect(14,y,182,38,2,2,'F');
      doc.setFillColor(251,191,36); doc.rect(14,y,3,38,'F');
      doc.setFontSize(9); doc.setTextColor(255,235,180);
      doc.text('PageSpeed data unavailable', 20, y+9);
      doc.setFontSize(7.5); doc.setTextColor(210,200,170);
      doc.text('The Google PageSpeed Insights API was rate-limited or unreachable during this audit.', 20, y+17);
      doc.text('This is temporary — run the audit again in 1-2 minutes to get Core Web Vitals data.', 20, y+24);
      doc.setTextColor(96,165,250);
      doc.text(`Check manually: https://pagespeed.web.dev/analysis?url=${encodeURIComponent(a.url)}`.slice(0,90), 20, y+32);
      y+=46;
    } else {
      // Performance score
      const pCol: [number,number,number] = scoreRgb(a.performance.performanceScore);
      doc.setFillColor(22,33,52); doc.roundedRect(14,y,182,22,3,3,'F');
      doc.setFillColor(...pCol); doc.rect(14,y,3,22,'F');
      doc.setFontSize(18); doc.setTextColor(...pCol); doc.text(String(a.performance.performanceScore),22,y+15);
      doc.setFontSize(8); doc.setTextColor(255,255,255); doc.text('/100 Desktop Performance Score (PageSpeed Insights)',40,y+12);
      if(a.mobile.available){ doc.setFontSize(7); doc.setTextColor(148,163,184); doc.text(`Mobile: ${a.mobile.mobileScore}/100`,40,y+19); }
      y+=28;

      // Core Web Vitals
      doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Core Web Vitals', 14, y); y+=6;
      const cwv: [string,string,string,string][] = [
        ['LCP — Largest Contentful Paint', `${(a.performance.lcp/1000).toFixed(2)}s`, a.performance.lcpRating, 'Good: under 2.5s'],
        ['CLS — Cumulative Layout Shift',  `${a.performance.cls}`,                    a.performance.clsRating,  'Good: under 0.1'],
        ['FID — Max Potential FID',        `${a.performance.fid}ms`,                  a.performance.fidRating,  'Good: under 100ms'],
        ['FCP — First Contentful Paint',   `${(a.performance.fcp/1000).toFixed(2)}s`, a.performance.fcpRating,  'Good: under 1.8s'],
        ['TTFB — Time to First Byte',      `${a.performance.ttfb}ms`,                 a.performance.ttfbRating, 'Good: under 800ms'],
        ['Speed Index',                    `${(a.performance.speedIndex/1000).toFixed(2)}s`, a.performance.speedIndex<=3400?'Good':'Needs Improvement','Good: under 3.4s'],
        ['Total Blocking Time',            `${a.performance.totalBlockingTime}ms`,    a.performance.totalBlockingTime<=200?'Good':'Needs Improvement','Good: under 200ms'],
      ];
      cwv.forEach(([label, value, rating, threshold]) => {
        const good = rating==='Good';
        const c: [number,number,number] = good?[52,211,153]:rating==='Needs Improvement'?[251,191,36]:[248,113,113];
        doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,11,1,1,'F');
        doc.setFillColor(...c); doc.rect(14,y-3,3,11,'F');
        doc.setTextColor(220,230,245); doc.setFontSize(7.5); doc.text(label,20,y+3);
        doc.setTextColor(...c); doc.text(value,120,y+3);
        doc.setTextColor(100,116,139); doc.setFontSize(6.5); doc.text(threshold,148,y+3);
        y+=13;
      });
      y+=4;

      // Mobile
      if(a.mobile.available){
        doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Mobile Usability', 14, y); y+=6;
        const mobRows: [string,string][] = [['Tap Targets',a.mobile.tapTargets],['Font Sizes',a.mobile.fontSizes],['Content Width',a.mobile.contentWidth]];
        mobRows.forEach(([label,value])=>{
          const good=value==='Pass'||value==='N/A'; const c:[number,number,number]=good?[52,211,153]:[248,113,113];
          doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,9,1,1,'F');
          doc.setFillColor(...c); doc.rect(14,y-3,2,9,'F');
          doc.setTextColor(148,163,184); doc.setFontSize(7); doc.text(label,19,y+2);
          doc.setTextColor(220,230,245); doc.text(value.slice(0,60),70,y+2);
          y+=11;
        });
        y+=4;
      }
      ...
      // Opportunities
      if(a.performance.opportunities.length>0){
        doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Optimisation Opportunities', 14, y); y+=6;
        a.performance.opportunities.forEach((opp: string)=>{
          doc.setFillColor(40,30,10); doc.roundedRect(14,y-3,182,9,1,1,'F');
          doc.setFillColor(251,191,36); doc.rect(14,y-3,2,9,'F');
          doc.setTextColor(220,230,245); doc.setFontSize(7.5); doc.text(decode(opp).slice(0,85),19,y+2); y+=11;
        });
      }
    }

    // ── PAGE 5: Internal Linking + Local SEO + Issues ──────────────────────
    newPage();
    doc.setFillColor(30,41,59); doc.rect(0,0,W,22,'F');
    doc.setFillColor(168,85,247); doc.rect(0,0,5,22,'F');
    doc.setTextColor(255,255,255); doc.setFontSize(13);
    doc.text('Internal Linking, Local SEO & Full Issue Log', 14, 14);
    y = 30;

    // Internal linking
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Internal Linking', 14, y); y+=6;
    const ilRows: [string,string,boolean][] = [
      ['Total Internal Links', String(a.internalLinking.totalInternalLinks), a.internalLinking.totalInternalLinks>3],
      ['Navigation (<nav>)',   a.internalLinking.hasNavigationLinks?'Present':'Not found', a.internalLinking.hasNavigationLinks],
      ['Breadcrumbs',          a.internalLinking.hasBreadcrumbs?'Detected':'Not found', a.internalLinking.hasBreadcrumbs],
      ['Footer Links',         a.internalLinking.hasFooterLinks?'Footer present':'No footer', a.internalLinking.hasFooterLinks],
    ];
    ilRows.forEach(([label,value,good])=>{
      const c:[number,number,number]=good?[52,211,153]:[248,113,113];
      doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,9,1,1,'F');
      doc.setFillColor(...c); doc.rect(14,y-3,2,9,'F');
      doc.setTextColor(148,163,184); doc.setFontSize(7); doc.text(label,19,y+2);
      doc.setTextColor(220,230,245); doc.text(value,80,y+2);
      y+=11;
    });

    // Sample internal links
    if(a.internalLinking.uniqueInternalLinks.length>0){
      y+=3; doc.setFontSize(7.5); doc.setTextColor(148,163,184); doc.text('Sample internal links found:', 14, y); y+=5;
      a.internalLinking.uniqueInternalLinks.slice(0,6).forEach((link: string)=>{
        doc.setTextColor(96,165,250); doc.setFontSize(7); doc.text(link.slice(0,75),18,y); y+=5;
      });
    }
    y+=6;

    // Local SEO
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text('Local SEO Signals', 14, y); y+=6;
    const localRows: [string,string,boolean][] = [
      ['Address / Location', a.localSeo.hasAddress?'Detected on page':'Not found', a.localSeo.hasAddress],
      ['Phone Number',       a.localSeo.hasPhone?'Detected on page':'Not found', a.localSeo.hasPhone],
      ['Email Address',      a.localSeo.hasEmail?'Detected on page':'Not found', a.localSeo.hasEmail],
      ['Map Embed',          a.localSeo.hasMap?'Map detected':'No map found', a.localSeo.hasMap],
      ['Business Hours',     a.localSeo.hasBusinessHours?'Hours mentioned':'Not found', a.localSeo.hasBusinessHours],
    ];
    localRows.forEach(([label,value,good])=>{
      const c:[number,number,number]=good?[52,211,153]:[148,163,184];
      doc.setFillColor(20,30,46); doc.roundedRect(14,y-3,182,9,1,1,'F');
      doc.setFillColor(...c); doc.rect(14,y-3,2,9,'F');
      doc.setTextColor(148,163,184); doc.setFontSize(7); doc.text(label,19,y+2);
      doc.setTextColor(220,230,245); doc.text(value,80,y+2);
    ...
    // Note about unsupported sections
    doc.setFillColor(18,26,40); doc.roundedRect(14,y,182,20,2,2,'F');
    doc.setFontSize(7); doc.setTextColor(80,100,130);
    doc.text('Sections not available (require paid third-party APIs):', 18, y+6);
    doc.text('Backlink Audit, Competitor Analysis, Keyword Rankings — require Ahrefs / SEMrush / Google Search Console API.', 18, y+12);
    doc.text('These tools start from $99/month. For free alternatives, use Google Search Console and Ubersuggest.', 18, y+17);
    y+=27;

    // ── Full Issue Log ─────────────────────────────────────────────────────
    if(y>230){ newPage(); y=20; }
    doc.setFontSize(9); doc.setTextColor(255,255,255); doc.text(`Complete Issue Log (${a.issues.critical.length + a.issues.warnings.length + a.issues.passed.length} checks)`, 14, y); y+=7;

    // Critical — with full detail + fix explanation
    if(a.issues.critical.length>0){
      doc.setFontSize(7.5); doc.setTextColor(248,113,113); doc.text(`CRITICAL (${a.issues.critical.length})`, 14, y); y+=5;
      a.issues.critical.forEach((iss: {item:string;detail:string;fix:string}) => {
        if(y>265){newPage();y=20;}
        const critPlaceholder = issueExplanations[iss.item] || iss.detail;
        const bh = 10 + wrap(critPlaceholder, 150, 7).length*5 + wrap(decode(iss.fix), 150, 7).length*5 + 14;
        doc.setFillColor(40,12,12); doc.roundedRect(14,y,182,bh,2,2,'F');
        doc.setFillColor(248,113,113); doc.rect(14,y,3,bh,'F');
        const critExplanation = issueExplanations[iss.item] || iss.detail;
        const critExpLines = wrap(decode(critExplanation), 150, 7);
        const critFixLines = wrap(decode(iss.fix), 150, 7);
        doc.setTextColor(255,200,200); doc.setFontSize(8.5); doc.text(decode(iss.item).slice(0,75), 20, y+8);
        let by = y+15;
        doc.setFontSize(6); doc.setTextColor(248,113,113); doc.text('WHY IT MATTERS', 20, by); by+=4;
        doc.setTextColor(210,180,180); doc.setFontSize(7);
        critExpLines.forEach((l:string)=>{doc.text(l,20,by);by+=5;}); by+=3;
        doc.setFontSize(6); doc.setTextColor(248,113,113); doc.text('HOW TO FIX', 20, by); by+=4;
        doc.setTextColor(210,180,180); doc.setFontSize(7);
        critFixLines.forEach((l:string)=>{doc.text(l,20,by);by+=5;});
        y+=bh+3;
      });
      y+=2;
    }

    // Warnings — with rich explanations
    const issueExplanations: Record<string,string> = {
      'Title length issue': 'Your page title is too long and will be cut off in Google search results, reducing click-through rates. Google displays titles up to 60 characters — anything beyond that is replaced with "...".',
      'Meta description length': 'Your meta description is outside the optimal 150-160 character range. Google may auto-generate its own snippet, which is often less compelling than a custom one.',
      'Missing meta description': 'No meta description was found. Google will auto-generate a snippet from your page content, which is often irrelevant and reduces click-through rates from search results.',
      'H1 tag issue': 'Your page has either no H1 tag or multiple H1 tags. Search engines use the H1 as the primary signal for what your page is about — it should appear exactly once.',
      'Missing canonical tag': 'Without a canonical tag, search engines may index multiple URL variants of your page (e.g. http vs https, with/without trailing slash), splitting your ranking authority across duplicates.',
      'Canonical points to wrong domain': 'Your canonical tag points to a different domain than the one being analysed (likely a staging or preview deployment URL). This tells Google that your real domain is just a duplicate of the staging site, which can prevent your actual domain from ranking at all.',
      'Sitemap points to wrong domain': 'Your sitemap references a different domain than the one being analysed (likely a staging or preview deployment URL). Search engines following this sitemap will index the wrong domain instead of your live site.',
      'No structured data': 'Structured data (Schema.org) tells search engines exactly what your content represents — a business, product, article, etc. Without it you miss eligibility for rich results like star ratings and FAQ dropdowns in Google.',
      'No Open Graph tags': 'Open Graph tags control the image, title and description shown when your page is shared on Facebook, LinkedIn and WhatsApp. Without them, social shares show a generic or broken preview.',
      'No Twitter Card tags': 'Twitter Card meta tags control how your page appears when shared on X/Twitter. Without them, links show as plain text with no image preview.',
      'Thin content': 'Your page has fewer than 300 words. Google considers pages with very little content as low-quality, which can suppress rankings. More meaningful content helps demonstrate expertise and relevance.',
      'Missing title tag': 'No title tag was found on this page. The title tag is the most important on-page SEO element — it appears as the headline in search results and is the primary ranking signal.',
      'No XML sitemap': 'No sitemap.xml was found. A sitemap tells search engines about all the pages on your site and helps them discover and index new content faster.',
      'Missing lang attribute': 'The <html> tag has no lang attribute. This helps search engines and screen readers identify the page language, impacting both SEO and accessibility.',
      'LCP needs improvement': 'Largest Contentful Paint measures how long users wait to see the main content. A slow LCP (over 2.5s) creates a poor first impression and is a direct Google ranking factor.',
      'CLS needs improvement': 'Cumulative Layout Shift measures visual stability — how much the page jumps around as it loads. High CLS frustrates users and is a direct Google Core Web Vital ranking factor.',
      'Slow server response': 'Your server is taking too long to respond. A slow TTFB delays every other resource on the page and signals poor hosting infrastructure to search engines.',
      'High blocking time': 'JavaScript is blocking the main thread for too long, delaying interactivity. This directly affects your INP (Interaction to Next Paint) Core Web Vital score.',
      'No nav element': 'No <nav> HTML element was found. Using semantic navigation elements helps search engines understand your site structure and improves accessibility.',
    };
    if(a.issues.warnings.length>0){
      doc.setFontSize(7.5); doc.setTextColor(251,191,36); doc.text(`WARNINGS (${a.issues.warnings.length})`, 14, y); y+=5;
      a.issues.warnings.forEach((iss: {item:string;detail:string;fix:string}) => {
        if(y>262){newPage();y=20;}
        const explanation = issueExplanations[iss.item] || iss.detail;
        const expLines  = wrap(decode(explanation), 150, 7);
        const fixLines  = wrap(decode(iss.fix), 150, 7);
        const bh = 10 + expLines.length*5 + fixLines.length*5 + 14;
        doc.setFillColor(40,30,10); doc.roundedRect(14,y,182,bh,2,2,'F');
        doc.setFillColor(251,191,36); doc.rect(14,y,3,bh,'F');
        doc.setTextColor(255,235,180); doc.setFontSize(8.5); doc.text(decode(iss.item).slice(0,75), 20, y+8);
        let by = y+15;
        doc.setFontSize(6); doc.setTextColor(251,191,36); doc.text('WHY IT MATTERS', 20, by); by+=4;
        doc.setTextColor(210,200,170); doc.setFontSize(7);
        expLines.forEach((l:string)=>{doc.text(l,20,by);by+=5;}); by+=3;
        doc.setFontSize(6); doc.setTextColor(251,191,36); doc.text('HOW TO FIX', 20, by); by+=4;
        doc.setTextColor(210,200,170); doc.setFontSize(7);
        fixLines.forEach((l:string)=>{doc.text(l,20,by);by+=5;});
        y+=bh+3;
      });
      y+=2;
    }
    ...
    // Passed
    if(a.issues.passed.length>0){
      if(y>255){newPage();y=20;}
      doc.setFontSize(7.5); doc.setTextColor(52,211,153); doc.text(`PASSED (${a.issues.passed.length})`, 14, y); y+=5;
      a.issues.passed.forEach((iss: {item:string;detail:string}) => {
        if(y>272){newPage();y=20;}
        const detText = decode(iss.detail);
        const detL = wrap(detText, 150, 6.5);
        const ph = 7 + detL.length*4.5;
        doc.setFillColor(12,40,28); doc.roundedRect(14,y,182,ph,1,1,'F');
        doc.setFillColor(52,211,153); doc.rect(14,y,2,ph,'F');
        doc.setTextColor(52,211,153); doc.setFontSize(7); doc.text(decode(iss.item), 19, y+6);
        doc.setTextColor(148,163,184); doc.setFontSize(6.5);
        detL.forEach((l:string,i:number)=>doc.text(l,19,y+10+i*4.5));
        y+=ph+2;
      });
      y+=4;
    }

    // Action plan on final page
    if(y>240){newPage();y=20;}
    doc.setDrawColor(40,55,75); doc.line(14,y,196,y); y+=8;
    doc.setFontSize(10); doc.setTextColor(255,255,255); doc.text('Action Plan', 14, y); y+=7;

    const highPri = a.issues.critical.map((i: {item:string;fix:string}) => `[CRITICAL] ${decode(i.fix)}`).slice(0,4);
    const medPri  = a.issues.warnings.map((i: {item:string;fix:string}) => `[WARNING] ${decode(i.fix)}`).slice(0,4);
    const longTerm = ['Build quality backlinks from industry-relevant websites.','Publish regular long-form content (1000+ words) targeting key search terms.','Set up Google Analytics 4 and Google Search Console if not already done.','Monitor Core Web Vitals monthly via PageSpeed Insights.'];

    const actionSections: [string,[number,number,number],string[]][] = [
      ['High Priority — Fix within 7 days',[248,113,113],highPri.length>0?highPri:['No critical issues found — great work!']],
      ['Medium Priority — Fix within 30 days',[251,191,36],medPri.length>0?medPri:['No warnings found.']],
      ['Long-term Recommendations',[96,165,250],longTerm],
    ];
    actionSections.forEach(([title,col,items])=>{
      const sectH = items.length*7+10;
      if(y+sectH+6>282){newPage();y=20;}
      doc.setFillColor(22,33,52); doc.roundedRect(14,y,182,sectH,2,2,'F');
      doc.setFillColor(...col); doc.rect(14,y,3,sectH,'F');
      doc.setFontSize(8); doc.setTextColor(255,255,255); doc.text(title,20,y+7);
      doc.setFontSize(7); doc.setTextColor(180,195,215);
      items.forEach((item,i)=>doc.text(item.slice(0,95),20,y+13+i*7));
      y+=sectH+5;
    });
    ...
    doc.save(`URLBoost_FullAudit_${hostname}_${new Date().toISOString().slice(0,10)}.pdf`);
  };
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-4">
            ...
            <img src="/logo.jpg" alt="URLBoost Pro" className="w-16 h-16 rounded-2xl object-cover shadow-lg shrink-0" />
            <div>
              <h1 className="text-3xl font-bold tracking-tight">URLBoost Pro</h1>
              <p className="text-amber-600 dark:text-amber-400 text-xs font-semibold mt-0.5">by Ambaig Designs</p>
              <p className="text-slate-500 dark:text-slate-400 mt-0.5 text-xs">SEO Analysis · Free Submissions · Tracker · PDF Reports</p>
            </div>
          </div>
          <button
            onClick={() => setDark(d => !d)}
            className="p-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm shrink-0 ml-4"
          ...
          <div className="space-y-4">
            <div className={`${card} p-8`}>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => void analyze()} disabled={loading || auditStep==='loading' || !url.trim()}
                  ...
                  {loading ? 'Analyzing…' : <><Search size={16} /> Quick SEO</>}
                </button>
              </div>
              {loading && (
                <div className="mt-4">
              ...
              <p className="text-xs text-slate-400 mt-4">Quick check: title, meta description, H1, alt text, viewport, schema.org, canonical, Open Graph.</p>
            </div>
            ...
            <div className={`${card} p-6`}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-base">Full SEO Audit Report</h3>
                    <span className="text-xs bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-medium">Powered by PageSpeed Insights</span>
                  </div>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Deep multi-page audit covering 10 SEO areas with PDF export.</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-4 text-xs text-slate-500 dark:text-slate-400">
                {['Executive Summary & Grade','Technical SEO (HTTPS, Sitemap, Canonical)','On-Page SEO (Title, H1-H6, Schema)','Core Web Vitals (LCP, CLS, FID, TTFB)','Mobile Usability Analysis','Internal Linking Structure','Local SEO Signals','Full Issue Log + Action Plan'].map(item => (
              ...
              {auditError && (
                <div className="bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-300 rounded-xl px-4 py-3 mb-3 text-sm flex items-start gap-2">
                  <XCircle size={15} className="shrink-0 mt-0.5" /> {auditError}
                </div>
              )}
              {auditStep === 'loading' && (
                <div className="mb-4">
                  <div className="flex justify-between text-xs text-slate-400 mb-1.5">
                    <span>Running full audit + PageSpeed Insights (takes 20-40 seconds)…</span>
                    <span>{auditProgress}%</span>
                  </div>
                  <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                    <div className="bg-gradient-to-r from-blue-500 to-emerald-500 h-2 rounded-full transition-all duration-300" style={{ width: `${auditProgress}%` }} />
                  </div>
                </div>
              )}
              {auditStep === 'done' && auditData && (
                <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/40 rounded-xl px-4 py-3 mb-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <CheckCircle size={16} className="text-emerald-500" />
                    <span className="text-sm font-medium text-emerald-700 dark:text-emerald-300">
                      Audit complete — Score: {(auditData as Record<string,{score:number}>).executive.score}/100 · Grade: {(auditData as Record<string,{grade:string}>).executive.grade}
                    </span>
                  </div>
                  <button onClick={downloadFullAudit} className="flex items-center gap-1.5 text-xs bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-lg font-medium transition-colors">
              ...
              <button
                ...
                disabled={auditStep==='loading' || loading || !url.trim()}
                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3.5 rounded-xl font-semibold text-sm transition-all"
              >
                {auditStep==='loading' ? (
                  <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Running Full Audit…</>
                ) : (
                  <><Search size={16} /> Run Full SEO Audit</>
                )}
              </button>
              <p className="text-xs text-slate-400 mt-2 text-center">Takes 20–40 seconds · Uses Google PageSpeed Insights API · Free</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={downloadReport} className="py-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2">
                <Download size={16} /> SEO Report (PDF)
              </button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5">
              ...
              <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  {(guideSite.fields.length > 1 || guideSite.requiresAccount) && (() => {
                    const fields: {id: string; label: string; value: string; empty?: boolean}[] = [];
                    ...
                    if (guideSite.fields.includes('email'))
                      fields.push({ id:'email', label:'Email Address', value: bizInfo.email || '', empty: !bizInfo.email });
                        ...
                        <div className="space-y-2">
                          {fields.map(f => (
                            <div key={f.id}>
                              ...
                              {f.empty ? (
                                <div className="flex items-center gap-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 rounded-lg px-3 py-2">
                                  <AlertTriangle size={13} className="text-amber-500 shrink-0" />
                                  <p className="text-xs text-amber-600 dark:text-amber-400">Add your email in Business Info above to pre-fill this.</p>
                                </div>
                              ) : (
            ...
            <div className="flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-5 py-3">
              ...
              <div className="flex items-center gap-3">
                {postedCount > 0 && (
                  <button onClick={clearHistory} className="text-xs text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors underline underline-offset-2">
                    Reset history
                  </button>
                )}
                <div className="flex items-center gap-1.5 text-xs text-slate-400">
                  <Shield size={13} className="text-emerald-500" /> Verified free
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={downloadSubmissionReport} className="py-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2">
                <Download size={16} /> Submission Report
              </button>
            </div>
          </div>
        )}
            ...
            <div className={`${card} p-8 text-center`}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button onClick={downloadReport}
                  className="inline-flex items-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-3.5 rounded-xl font-semibold hover:opacity-90 transition-opacity text-sm">
                  <Download size={18} /> SEO Report (PDF)
                </button>
                <button onClick={downloadSubmissionReport}
                  className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3.5 rounded-xl font-semibold transition-colors text-sm">
                  <Download size={18} /> Submission Report (PDF)
                </button>
              </div>
            </div>
